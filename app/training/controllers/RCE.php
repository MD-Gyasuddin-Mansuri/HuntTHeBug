...
class RCE
{
    ...
    {
        if (
            file_exists(getcwd() . "/../data/blogcomments/" . $arg[1] . ".txt")
        ) {
    ...
    public static function lesson3()
    {
        if (isset($_POST["comment"])) {
            file_put_contents(
                getcwd() . "/../data/blogcomments/" . date("U") . ".txt",
                $_POST["comment"]
            );
            \View::redirect("/");
        }
    ...
    {
        echo exec(
            'curl -H "Host: rce.huntthebug.sec" http://127.0.0.1/stock/?id=' .
                $_GET["product_id"]
        );
    }
    ...
    public static function lesson1Stock()
    {
        if ($_GET["id"] == 1) {
            ...
            \Output::error(["Could not find product ID: " . $_GET["id"]], 404);
        }
    }
}
