...
class CSRF
{
    ...
    {
        $json = json_decode(
            '{"admin":{"username":"admin","password":"admin","email":"admin@admin.test","notifications":false},"eve":{"username":"eve","password":"eve","email":"eve@adam.test","notifications":false}}',
            true
        );
        self::setData($json);
        \View::redirect("/");
    ...
    public static function home()
    {
        if (
            isset($_COOKIE["token"]) &&
            ($_COOKIE["token"] == "9738EFC3561120092F7AB66514547475" ||
                $_COOKIE["token"] == "649EFEE221D752E4E943D7811B95408E")
        ) {
            \View::page("csrf/dash");
        } else {
            \View::redirect("/login");
            ...
            $data = [
                ...
                "csrf" => base64_encode(
                    json_encode([
                        "data" => $datastr,
                        "signature" => md5(
                            "the s3cr3t s4lt" . md5(json_encode($datastr))
                        ),
                    ])
                ),
            ];
    ...
    public static function login()
    {
        ...
        if (isset($_POST["username"], $_POST["password"])) {
            ...
            if (
                ...
                $users["admin"]["password"] == $_POST["password"]
            ) {
                setcookie(
                    "token",
                    "9738EFC3561120092F7AB66514547475",
                    time() + 3600,
            ...
            if (
                $_POST["username"] === "ben" &&
                $users["ben"]["password"] == $_POST["password"]
            ) {
                setcookie(
                    "token",
                    "649EFEE221D752E4E943D7811B95408E",
                    time() + 3600,
                    "/; Secure; SameSite=None"
                );
                \View::redirect("/");
