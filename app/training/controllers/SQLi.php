...
class SQLi
{
    private static $sql_username = "huntthebug";
    private static $sql_password = "GtWKcpX3MAmCviGC";
    private static $db = "huntthebug";
    ...
    public static function articleCount()
    {
        ...
        $pdo = new \PDO(
            "mysql:host=127.0.0.1;port=3306;dbname=" . self::$db,
            self::$sql_username,
            self::$sql_password
        );
        $d = $pdo->prepare(
            "select * from article where created_at like '%" .
                $_GET["date"] .
                "'  "
        );
        $d->execute([]);
